...
strPath = '/PATH/TO/UCF101/ucf101_interp_ours/'
...
if __name__ == '__main__':
    ...
    for strDir in tqdm.tqdm(lstDirs):
        npyOne = numpy.array(PIL.Image.open(strPath + strDir + '/frame_00.png'))[:, :, ::-1].astype(numpy.float32) * (1.0 / 255.0)
        npyTwo = numpy.array(PIL.Image.open(strPath + strDir + '/frame_02.png'))[:, :, ::-1].astype(numpy.float32) * (1.0 / 255.0)
        npyTruth = numpy.array(PIL.Image.open(strPath + strDir + '/frame_01_gt.png'))[:, :, ::-1].astype(numpy.float32) * (1.0 / 255.0)
